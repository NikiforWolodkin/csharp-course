...
namespace Lab_6
{
    ...
    public class Session
    {
        ...
        {
            Debug.Assert(Exams.Contains(exam), "Попытка удалить несуществующий элемент из списка.");
            ...
            {
                throw new SessionException("Данного экзамена нет в сессии!", exam);
            }    
        }
    ...
    public static partial class Examination
    {
        ...
        abstract public class ExaminationChallenge
        {
            ...
            public ExaminationChallenge(int amountOfQuestions, int allowedMistakes)
            {
                ...
                {
                    throw new ExaminationArgumentException("Должен быть хотя бы один вопрос!", amountOfQuestions);
                }
                else if (amountOfQuestions > s_questionSet.Length)
                {
                    throw new QuestionArgumentException("В каталоге вопрос не хватает вопросов!", amountOfQuestions);
                }
                else
                {
                ...
                {
                    throw new ExaminationArgumentException("Число допустимых ошибок не может быть отрицательным!", allowedMistakes);
                }
                else
                {
